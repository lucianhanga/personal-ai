...
MCP servers (M6, e.g. Microsoft Playwright MCP) or a future multi-user deployment.
...
- **Tier 1 — subprocess (M6):** resource/network-limited child process; the default for spawning
  **MCP servers over stdio** (which are out-of-process by protocol regardless).
- **Tier 2 — container/microVM (M6+):** strongest isolation for untrusted/heavy MCP servers
...
- Negative: tier 0 offers no OS-level isolation, so only *trusted* code may run in-process; running
  untrusted tools requires tier 1/2, which lands with MCP in M6 (tracked, not optional).
