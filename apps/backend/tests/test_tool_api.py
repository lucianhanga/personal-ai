...
def test_list_tools() -> None:
    client = _client()
    data = client.get("/api/v1/tools", headers=AUTH).json()["data"]["tools"]
...
def test_tools_require_token() -> None:
    assert _client().get("/api/v1/tools").status_code == 401
...
def test_tool_log_records_calls() -> None:
    ...
    entries = client.get("/api/v1/tools/log", headers=AUTH).json()["data"]["entries"]
...
def test_tool_log_requires_token() -> None:
    assert _client().get("/api/v1/tools/log").status_code == 401
